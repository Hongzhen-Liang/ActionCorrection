...
# 读取数据
for i in range(3):
    Right.append(pd.read_csv('./右侧幻椅式/右侧幻椅式00'+str(i)+'.csv'))
    Wrong.append(pd.read_csv('./右侧幻椅式/错右侧幻椅式00'+str(i)+'.csv'))
    Right[i].columns = ['ch1']
    Wrong[i].columns = ['ch1']
plot_photo(Right[0],'RightMagicChairTwist')
plot_photo(get_move_window(Right[0],windowlength),'RightMagicChairTwist_move')
# plot_photo(Wrong[0],'RightMagicChairTwist_Wrong','maroon')
# plot_photo(get_move_window(Wrong[0],windowlength),'RightMagicChairTwist_Wrong_move','maroon')
# plt.show()
...
mav_Right['action']='右侧幻椅式扭转:挺直腰背,动作正确'
mav_Wrong['action']='右侧幻椅式扭转:弯曲腰背,动作错误'
...
joblib.dump(model, "RightMagicChairTwist.m")  # 保存模型
